...
// Manages activation, partial sum, and fully systolic weight dataflows.
...
logic weight_write_wire [0:8][0:7];
logic weight_swap_wire [0:8][0:8];
logic weight_swap_col0 [1:7];

// Connects external ports to boundary wires and maps array outputs.
generate
    for (genvar i = 0; i < 8; i++) begin : boundary_connections
        ...
        assign weight_write_wire[0][i] = weight_write_in;
...
// Generates the 2D array of processing elements and chains their connections.
generate
    for (genvar row = 0; row < 8; row++) begin : tile_row
        for (genvar col = 0; col < 8; col++) begin : tile_col
            pe pe_inst (
                ...
                .weight_write_in(weight_write_wire[row][col]),
                .weight_write_out(weight_write_wire[row+1][col]),
...
// Connects the final boundary control signals to the module outputs.
assign weight_write_out = weight_write_wire[8][7];
